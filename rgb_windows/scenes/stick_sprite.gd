...
@export var maxLength = 16
@export var deadzone = 6
...
func _process(delta: float) -> void:
	if pressing:
		if get_global_mouse_position().distance_to(controller.global_position) <= deadzone:
			global_position = controller.global_position
		elif get_global_mouse_position().distance_to(controller.global_position) <= maxLength:
